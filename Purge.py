   ...
   with lock:
    ...
    for elem in Elems: 
     ...
     if search('friends_tab',Link): 
      ...
      with open('/tmp/Blocked','r') as Rd:
       for name in Rd:
        if str(name).strip() == friend: Found_Friend=True;break 
      if not Found_Friend and friend != 'profile.php'.strip(): 
       with open('/tmp/Blocked','a') as B_List:Friends.append(friend);B_List.write('{}\n'.format(friend));
   ...
   with lock:
    ...
    for elem in Elems: 
     ...
     if search('photo.php',Link): 
      ...
      with open('/tmp/Deleted','r') as R_L:
       for i in R_L:
        if str(i).strip() == photo: Photo_Found = True
      if not Photo_Found: 
       with open ('/tmp/Deleted','a') as Pics: Photos.append(photo);Pics.write('{}\n'.format(photo))
...
def Delete_Photos():
 ...
 with lock:
  ...
  for photo in Photos:
   try:
    call(['clear']);deleted+=1;print '[+] Deleting Photos...\n[+] Deleted Photos: {}'.format(deleted)
...
if __name__ == '__main__': 
 ...
 On    = True
 Check_list()
 if path.exists('/tmp/Deleted'): 
  remove('/tmp/Deleted'); 
  with open('/tmp/Deleted','a'): pass
 else: 
  with open('/tmp/Deleted','a'): pass

 if path.exists('/tmp/Blocked'): 
  remove('/tmp/Blocked'); 
  with open('/tmp/Blocked','a'): pass
 else: 
  with open('/tmp/Blocked','a'): pass
 ...
 try:
  ...
  if not Run and not Run2:  
    try: remove('/tmp/Blocked');remove('/tmp/Deleted');remove('ghostdriver.log')
    except: pass
