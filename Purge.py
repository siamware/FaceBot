...
finally: from selenium import webdriver;from selenium.webdriver.common.keys import Keys
#
	 ...
	 if len(Photos) == 0:
		  ...
		  if Spin2 >= 20: driver.execute_script("window.scrollTo(0, document.body.scrollHeight);");Spin2=0
			...
			if search('photo.php',Link): 
				...
				if not search(photo,str(Deleted)):
				 Photos.append(photo);Deleted.append(photo)
...
def Block_Friends():
	...
	with lock:
	 ...
	 for friend in Friends:
	    ...
	    span_xpath = '//span[contains(text(), "Block")]'
	    span_element = driver2.find_element_by_xpath(span_xpath).click();sleep(1.7)
	    Block = "//button[@class='_42ft _4jy0 layerConfirm uiOverlayButton _4jy3 _4jy1 selected _51sy']"
	    driver2.find_element_by_xpath(Block).click()
	...
	try:
		...
		while Run:
		   if len(Friends) != 0:
			...
			call(['clear']);print '[-] Waiting for friends\' list to populate...';sleep(1)
			Spin+=1
		...
		while Run2:
			...
			call(['clear']);print '[-] Waiting for photos\' list to populate...';sleep(1)
			Spin2+=1
